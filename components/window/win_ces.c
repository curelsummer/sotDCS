...
static void show_label_width(void);
...
static void show_width(sel_state_t sel);
...
static void set_width(int32_t type, float value);
...
static void show_label_width(void)
{
    uint16_t cstr[] = { 9, 10, 0 };
    show_label_cstr(cstr, ROW_WIDTH, 0, PIC);
    show_unit_astr("us", ROW_WIDTH, 0, PIC);
}
...
{
    float freq;
    int32_t err = ces_get_freq(wave, &freq);
    if (err < 0)
    {
        ESP_LOGW(TAG, "ces_get_freq err. <%d>", err);
        return;
    }

    char str[16];
    sprintf(str, "%5.0f", freq);
...
{
    float width;
    int32_t err = ces_get_width(wave, &width);
    if (err < 0)
    {
        ESP_LOGW(TAG, "ces_get_width err. <%d>", err);
        return;
    }

    char str[16];
    sprintf(str, "%5.0f", width);
    show_value_astr(str, ROW_WIDTH, sel, PIC);
}
...
static void set_freq(int32_t type, float freq)
{
    ...
    ces_set_freq(wave, value);
}

static void set_width(int32_t type, float width)
{
    float value;

    if (type == 1)
    {
        ces_get_width(wave, &value);
        value += width;
    }
    else
    {
